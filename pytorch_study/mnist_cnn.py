...
# 读取数据
datasets1=datasets.MNIST('./data',train=True,download=True,transform=transform)#训练的数据集
datasets2=datasets.MNIST('./data',train=False,download=True,transform=transform)#不是训练的数据集
...
class Net(nn.Module):
    def __init__(self):
        ...
        # 第一个参数表示输入的通道数,,由于是黑白图所以是1
        # 第二个参数选择输出多少个通道,其实就是选多少个卷积核
        # 第三个参数表示卷积核的大小  第四个表示步长

        self.conv1=nn.Conv2d(1,64,3,1)
        # 经过第一个卷积层后得到的FeatureMap是64,因此第二个卷积层的输入是64
        #Tip:池化不改变通道数
    ...
    def forward(self,x):
        x=self.conv1(x)
        x=F.relu(x)
        # 表示池化核的大小是2*2   步长默认和池化核一致,,,也是2*2的
        x=F.max_pool2d(x,2)
        ...
        # 对于一般的CNN框架来说,下一步应该把数据从4D转化为2D,,下面的表示从3D转化为1D
        # (这里的转化可以理解为把RGB转为一维向量,3D->1D表示1个RGB->一维向量,4D->2D就是多个RGB->多个一维向量,,)
        # 把每条数据变成一维数组
        x=torch.flatten(x,1)
        # 经过一个全连接
        # fc1表示求和
